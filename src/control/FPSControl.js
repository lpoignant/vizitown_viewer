...
FPSControl.prototype.update = function(delta) {
	...
	if (this.moveForward) this._camera.translateZ(-actualMoveSpeed);
	if (this.moveBackward) this._camera.translateZ(actualMoveSpeed);
	
	if (this.moveLeft) this._camera.translateX(-actualMoveSpeed);
	if (this.moveRight) this._camera.translateX(actualMoveSpeed);
	
	if (this.moveUp) this._camera.translateY(actualMoveSpeed);
	if (this.moveDown) this._camera.translateY(-actualMoveSpeed);
	
	//Rotation
	if (!this.mouseDragOn) return;
	var actualLookSpeed = delta * this.lookSpeed;
	var lon = -THREE.Math.degToRad(this.mouseX * actualLookSpeed) / 100;
	var lat = THREE.Math.degToRad(this.mouseY * actualLookSpeed) / 100;
